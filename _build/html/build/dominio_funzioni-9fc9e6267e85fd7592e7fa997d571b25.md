...
L’albero seguente mostra la classificazione generale; la tabella riassume per ogni famiglia un esempio tipico e la condizione che determina il dominio.

```{table} Esempi di funzioni elementari e relativi domini
:widths: auto
:align: center

| Classe | Sottoclasse | Esempio | Dominio |
|---|---|---|---|
| Algebriche | Razionali intere | $y = x^2 - 3x + 1$ | $\mathbb{R}$ |
| Algebriche | Razionali fratte | $y = \dfrac{1}{x - 2}$ | $\mathbb{R} \setminus \{2\}$ |
| Algebriche | Irrazionali di indice pari | $y = \sqrt{x + 4}$ | $x \geq -4$ |
| Algebriche | Irrazionali di indice dispari | $y = \sqrt{x - 1}$ | $\mathbb{R}$ |
| Trascendenti | Esponenziali | $y = e^{2x}$ | $\mathbb{R}$ |
| Trascendenti | Logaritmiche | $y = \ln(x - 5)$ | $x > 5$ |
| Trascendenti | Goniometriche | $y = \cos x$ | $\mathbb{R}$ |
```
