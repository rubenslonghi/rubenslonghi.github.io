...
left_x = [2.9, 2.99, 2.999]
right_x = [3.001, 3.01, 3.1]

left_fx = [2*x for x in left_x]
right_fx = [2*x for x in right_x]

df = pd.DataFrame({
    "Sinistra: x": left_x,
    "Sinistra: f(x)": left_fx,
    "Punto": [3, "indet.", ""],
    "Destra: x": right_x,
    "Destra: f(x)": right_fx,
}, index=["vicino 1", "vicino 2", "vicino 3"])
