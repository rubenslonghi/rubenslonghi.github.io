...
```{mermaid}
%% Diagramma ad albero: classificazione delle funzioni analitiche
flowchart TD

    F["FUNZIONI ANALITICHE"] --> A["FUNZIONI ALGEBRICHE"]
    F --> T["FUNZIONI TRASCENDENTI"]

    %% Ramo algebraiche
    A --> R["Razionali"]
    A --> IRR["Irrazionali"]

    R --> RI["Razionali intere<br/>\\(y = x^2 - 3x + 1\\)"]
    R --> RF["Razionali fratte<br/>\\(y = \\dfrac{1}{x-2}\\)"]

    IRR --> IP["Irrazionali di indice pari<br/>\\(y = \\sqrt{x+4}\\)"]
    IRR --> ID["Irrazionali di indice dispari<br/>\\(y = \\sqrt{x-1}\\)"]

    %% Ramo trascendenti
    T --> E["Esponenziali<br/>\\(y = e^{2x}\\)"]
    T --> L["Logaritmiche<br/>\\(y = \\ln(x-5)\\)"]
    T --> G["Goniometriche<br/>\\(y = \\cos x\\)"]

    %% Esempi di dominio
    RI --> D1["Dominio: \\(\\mathbb{R}\\)"]
    RF --> D2["Dominio: \\(\\mathbb{R} \\setminus \\{2\\}\\)"]
    IP --> D3["Dominio: \\(x+4 \\ge 0\\)"]
    ID --> D4["Dominio: \\(\\mathbb{R}\\)"]
    E --> D5["Dominio: \\(\\mathbb{R}\\)"]
    L --> D6["Dominio: \\(x-5 > 0\\)"]
    G --> D7["Dominio: \\(\\mathbb{R}\\)"]

    %% Stili
    classDef main fill:#1F2D3D,stroke:#1F2D3D,stroke-width:2px,color:#ffffff,font-weight:bold;
    classDef alg fill:#1F77B4,stroke:#1F77B4,stroke-width:2px,color:#ffffff,font-weight:bold;
    classDef tra fill:#8E44AD,stroke:#8E44AD,stroke-width:2px,color:#ffffff,font-weight:bold;
    classDef cat fill:#EAF2F8,stroke:#5D6D7E,stroke-width:1.5px,color:#1B2631;
    classDef ex fill:#FDFEFE,stroke:#AAB7B8,stroke-width:1px,color:#212F3D;
    classDef dom fill:#FCF3CF,stroke:#D4AC0D,stroke-width:1px,color:#7D6608;

    class F main;
    class A alg;
    class T tra;
    class R,IRR cat;
    class RI,RF,IP,ID,E,L,G ex;
